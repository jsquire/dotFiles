...
6. Reboot **only if** the running kernel's modules directory has disappeared, or CachyOS's own `cachyos-reboot-required.hook` flagged the upgrade.
...
* **Reboot detection is delegated, not reimplemented.**  `cachyos-hooks` ships `cachyos-reboot-required.hook`, which is the distro's own authoritative answer to "does this upgrade need a reboot".  It covers far more than a hand-maintained list would (microcode, `mesa`, `cryptsetup`, `mkinitcpio`, `dracut`, `systemd*`, `wayland`, `xorg*`, `linux-firmware`) and is smarter: it fires only for the *running* kernel, so an alternate-kernel upgrade is ignored, and `btrfs-progs` only counts when btrfs is actually mounted.  Notably it does **not** treat `linux-headers` as reboot-worthy, which a naive `linux*` match would.  The hook keeps no state file and its desktop notification only reaches logged-in users, so the job records `wc -l` of `/var/log/pacman.log` before upgrading and afterwards inspects only the lines this run appended for the hook's `Reboot is recommended` message.  Nothing rotates `pacman.log` on this host today, but if a `logrotate` stanza is ever added, a file that has *shrunk* since the mark was taken is treated as proof of a mid-run rotation and the whole file is rescanned; an unnecessary reboot is a far cheaper mistake than a stale kernel running against a mismatched `nvidia.ko`.
* **Reboot safety.**  The job refuses to reboot if `sshd` is not active. Autologin is enabled, so KRDP recovers once the Plasma session starts after a reboot; however, during early boot and on a Plasma crash, SSH is the only remote recovery path. Cleanup deliberately runs *before* the reboot, because a reboot terminates the script.
...
* **Editors and toolchains come from the package manager.**  `micro` and `nvm` are installed with `pacman` rather than piped from `getmic.ro` and `raw.githubusercontent.com` into `sudo bash`.  This matters beyond provenance: an unmanaged binary in `/usr/local/bin` *shadows* the packaged one in `PATH`, so the weekly `pacman -Syu` above would faithfully update a binary that never actually runs.  Packaged `nvm` also drops the hard-coded version pin, so each run installs whatever is current.  Because the packaged `nvm` lives at `/usr/share/nvm/init-nvm.sh`, `.zshrc` prefers that path and falls back to `~/.nvm` only if it is absent.  If a legacy curl/wget install is detected, `bootstrap.sh` prints a one-time notice telling you what to remove; it does not delete anything itself, and your installed Node versions under `~/.nvm/versions` are left untouched.  Subsequent runs simply use the packaged versions.
