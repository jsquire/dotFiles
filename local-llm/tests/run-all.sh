#!/usr/bin/env bash
# Orchestrator for the bash-side suites (schema, daemon, launcher parity, token budgets, installer
# generation). Prints a per-suite result and an isolation check, and exits nonzero if anything failed.
set -uo pipefail
...
fail=0
for s in test_schema.sh test_daemon.sh test_launchers_parity.sh test_token_budgets.sh test_installer_gen.sh test_imagegen_context.sh test_imagegen_path.sh test_office_skill.sh test_ollama_proxy.sh; do
    echo "########## $s ##########"
