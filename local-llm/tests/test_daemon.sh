...
assert_eq "GET /models mode count" "4" "$(jq_count <<<"$models")"
...
assert_eq "GET /models includes task menu" "5" "$(python3 -c 'import json,sys;d=json.load(sys.stdin);print(sum(len(c["rows"]) for c in d["menu"]["categories"]))' <<<"$models")"
...
assert_eq "fallback roster 4 modes" "4" "$(jq_count <<<"$fb")"
kill "$DPID" 2>/dev/null; wait "$DPID" 2>/dev/null || true; DPID=""
...
assert_eq "malformed -> fallback 4 modes" "4" "$(jq_count <<<"$mf")"
kill "$DPID" 2>/dev/null; wait "$DPID" 2>/dev/null || true; DPID=""
rm -f "$bad"
